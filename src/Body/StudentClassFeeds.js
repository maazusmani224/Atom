...
import {Card,Transition,Header,Icon,Button,Modal,Label,Divider,Placeholder} from 'semantic-ui-react'
...
export default function StudentClassFeeds(props){
  const [tests,setTests] = useState([])
    ...
    useEffect(()=>{
        const unsubs_post = db.collection('posts').where('class','==',props.classid).orderBy('created','desc').onSnapshot(querysnapshot=>{
            setPosts(querysnapshot.docs.map(doc=>({id:doc.id,data: setDate(doc.data())})));
        })
    ...
    function leaveClass(){
      db.collection('partof').where('student','==',user.email).where('class','==',props.classid).get().then(query=>{
        query.docs[0].ref.delete().then(()=>console.log("deleted")).catch(err=>console.log(err));
        })
    ...
    return (
        ...
        <div className="backbutton__container">
             ...
             <Label color='green' size='large'>{classname}<Label.Detail><Icon name='clipboard list' size='large'/>{posts.length}</Label.Detail></Label>
            ...
            open={leaveclassopen}
            size='small'
            trigger={<Button className="leaveclassbutton" basic color="red">Leave Class</Button>}
          >
            <Header icon>
              <Icon name='sign-out' />
              Leave Class
            ...
            <Modal.Actions>
              <Button basic color='green' inverted onClick={() => setLeaveClassOpen(false)}>
                <Icon name='remove' /> No
              </Button>
        ...
        >
                ...
                <Card key={post.id} id={post.id}>
                  ...
                  <Card.Content description={post.data.description}/>
                      {post.data.fileurl?<Card.Content extra>
                    <a style={{margin:'0 20% 0 0'}} classname="postfile__download" href={post.data.fileurl}><Icon name='download' color='green' size='large'/>{post.data.filename}</a></Card.Content>:""}
                </Card>
              ))
              }</Transition.Group>}
        </TabPanel>
