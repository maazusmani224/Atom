...
import { Divider,Card,Button,Icon,Input,Transition,CardGroup,Label,Placeholder } from 'semantic-ui-react';
...
'linear-gradient(to right, #141e30, #243b55)','linear-gradient(to right, #200122, #6f0000)','linear-gradient(to right, #536976, #292e49)','linear-gradient(to right, #bdc3c7, #2c3e50)',]
    ...
    function joinClass(){
        if(inputclasscode){
            db.collection('classes').doc(inputclasscode).get().then(doc=>{
                if(doc.exists){
                    db.collection('partof').where('class','==',inputclasscode).where('student','==',user.email).get()
                    .then(query=>{
                       if(query.empty){
                          db.collection('partof').add({
                            class: inputclasscode,
                            student: user.email,
                            joined: firebase.default.firestore.FieldValue.serverTimestamp()
                         })
                       }
                       else{
    ...
    return(
        ...
        <div className="studentclass__classes">
            <Snackbar anchorOrigin={{vertical:'top',horizontal:'center'}} open={snackbaropen} autoHideDuration={3000} onClose={handleSnackBarClose}>
              <MuiAlert elevation={5} variant='filled' onClose={handleSnackBarClose} severity={severity}>
               {errmessage}
              </MuiAlert>
            </Snackbar>
            {props.loading?<Card.Group>{[1,1,1].map(elem=>(<Card className="loading__card">
                        <Card.Content style={{background:gradients[0]}}><Card.Header><Label color={getRandomColorForLabel()} size='medium' ><Placeholder><Placeholder.Line/></Placeholder><Label.Detail><Placeholder><Placeholder.Line/></Placeholder></Label.Detail></Label></Card.Header></Card.Content>
                        <Card.Content><Card.Meta><Placeholder><Placeholder.Line/></Placeholder></Card.Meta></Card.Content>
        ...
        >
            <Card key="addnew">
                      ...
                      <Card.Content><Input focus value={inputclasscode} onChange={getInputClassCode} placeholder='Class Code...' /></Card.Content>
                      <Card.Content extra><Button basic color='green' onClick={joinClass}><Icon name='add'/>Join Class</Button></Card.Content>
                    </Card>
                {classes.map(doc=>(
                    <Card key={doc.id} className="classes__card" onClick={openClass} id={doc.id}>
                        <Card.Content style={{background:gradients[0]}}><Card.Header><Label as='a' color={getRandomColorForLabel()} size='medium' image><img src={doc.creator.photoUrl}/>{doc.creator.username}<Label.Detail>{doc.data.title}</Label.Detail></Label></Card.Header></Card.Content>
