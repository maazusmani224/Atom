...
export default function TeacherClass(props) {
  ...
  useEffect(() => {
    const unsubscribe = db
      .collection("classes")
      .where("by", "==", auth.currentUser.email)
  ...
  return (
    <div className="teacherclass__main">
        ...
        {props.loading ? (
          <Card.Group centered>
            {[1, 1, 1].map((elem, index) => (
              <Card key={index}>
                ...
                >
                  <Card.Header>
                    <Label as="a" color={getRandomColorForLabel()} size="large">
                      <Placeholder>
                        <Placeholder.Line />
                      </Placeholder>
                    </Label>
          ...
          <Transition.Group as={CardGroup} duration={400} animation="pulse" centered={true}>
            <Card key="addnew">
              ...
              <Card.Content>
                <span className="addclass__button" onClick={openAddClassForm}>
      ...
      <div className="teacherclass__addclassform">
        <Dialog
          open={openaddclass}
          onClose={closeAddClassForm}
          aria-labelledby="form-dialog-addclass"
        >
