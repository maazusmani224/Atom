  ...
  return (
    <div className="question">
      ...
      <div className="question__text">
        <Message.Header>{props.index+1+".    "}{props.question.ques}</Message.Header>
      </div>
      {props.question.optiona&&<div><p style={{color:props.correctans===0&&'green'}}>{props.question.optiona}</p></div>}
      {props.question.optionb&&<div><p style={{color:props.correctans===1&&'green'}}>{props.question.optionb}</p></div>}
      {props.question.optionc&&<div><p style={{color:props.correctans===2&&'green'}}>{props.question.optionc}</p></div>}
      {props.question.optiond&&<div><p style={{color:props.correctans===3&&'green'}}>{props.question.optiond}</p></div>}
      {props.question.points&&<div><p>Points: {props.question.points}</p></div>}
      <div><Button basic color='red' onClick={deleteQues}><Icon name="delete"/>Delete</Button></div>
      </Message>
      <Divider />
    </div>
  )
...
export default function TeacherTests(props) {
  ...
  const [testopen,setTestOpen] = useState(false)
    ...
    function changeTestStatus(id,active){
      db.collection('tests').doc(id).get().then(snapshot=>{
        var active = snapshot.data().active;
        db.collection('tests').doc(id).update({active:!active}).catch(err=>console.log(err));
      }).catch(err=>console.log(err))
    }
      ...
      >
        <Segment className="create__test__form">
          ...
          {addques&&<div className="add__newques" style={{display:'flex',flexDirection:'column'}}>
            ...
            <div className="add__newques__elem"><Radio value={0} onChange={()=>{setCurrentCorrectAns(0)}} checked={currentcorrectans===0}/><Input type="text" placeholder="Option 1" value={currentques.optiona} onChange={(e,{value})=>setCurrentQues(prev=>{return {...prev,optiona:value}})}/></div>
            <div className="add__newques__elem"><Radio value={1} onChange={()=>{setCurrentCorrectAns(1)}} checked={currentcorrectans===1}/><Input type='text' placeholder="Option 2" value={currentques.optionb} onChange={(e,{value})=>setCurrentQues(prev=>{return {...prev,optionb:value}})}/></div>
